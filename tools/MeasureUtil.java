...
public class MeasureUtil {
	...
	public static double muInfor_log(String [] targets, String [] labels){
		...
		// all targets
		HashSet<String> target_set = new HashSet<String>();
		for(String target: targets)
			target_set.add(target);
		// iterative
		for (String target: target_set){
			// all labels for this target
			HashMap<String, Integer> hm_label=new HashMap<String, Integer>();
			...
			// max label in this label
			String tostr = "";
			int num = 0;
			for (String label: hm_label.keySet())
				if (hm_label.get(label)>num)
					tostr = label;
			// replace
			for(int i=0;i<targets.length;i++){
				if (targets[i]!=target)
		...
		return muInfor_log_(targets, labels);
	
	}
	
	// logistic mutual information
	private static double muInfor_log_(String [] targets, String [] labels){
		if (targets.length!=labels.length){
			System.out.println("len(targets)!=len(labels)");
			System.exit(1);
		}
